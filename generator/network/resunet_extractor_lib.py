...
class ResUNet(tf.keras.Model):
  ...
  def __init__(self,
               filters_sequence: Sequence[int] = (16, 32, 64, 96, 128, 128,
                                                  196),
               output_channel: int = 16,
               num_resblock_per_level: int = 2,
               skip_link_cutoff_level: int = 0,
               norm_fn: str = 'instance',
    ...
    Args:
      ...
      skip_link_cutoff_level: An integer specifying the level where [0, level]
        doesn't have skip link layers.
      norm_fn: A string specifying the type of normalization layer.
    ...
    self._skip_link_cutoff_level = skip_link_cutoff_level
    beta_initializer = 'zeros'
    gamma_initializer = 'ones'
  ...
  def call(
    ...
    for idx, layer in enumerate(self.decode_layers):
      ...
      if idx < len(self.decode_layers) - self._skip_link_cutoff_level:
